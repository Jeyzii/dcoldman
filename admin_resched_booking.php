...
if (isset($_GET['booking_id'])) {
    ...
    // Query to retrieve booking information along with user_id
    $query = "SELECT bookings.*, users.user_id 
                FROM bookings 
                INNER JOIN users ON bookings.user_id = users.user_id 
                WHERE bookings.booking_id = '$booking_id' 
                AND (bookings.status = 'resched' OR bookings.status = 'pending')";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        header("Location: admin_resched_bookings.php");
        exit;
    }
} else {
...
<body>
    ...
    <div class="container mt-5">
        ...
        <!-- Booking Edit Form -->
        <form action="backend/admin_resched_booking_process.php" method="post">
